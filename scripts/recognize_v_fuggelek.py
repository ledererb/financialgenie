#!/usr/bin/env python3
"""
Context-alapú mezőfelismerés generikus AcroForm mezőkkel rendelkező PDF-ekhez.

A V. számú függelék 96 AcroForm mezőjéből ~82 teljesen generikus
("Jelölőnégyzet NNN", "Szövegmező NNN"), ezért a név-alapú heurisztika
nem működik. Ez a script:

  1. pikepdf-del kinyeri a mezőneveket + oldalukat.
  ...
  3. Oldalanként batch-ben elküldi DeepSeek-nek, hogy klasszifikálja
     a mezőket a kanonikus modellre (vagy "unknown"-ra).
  4. A már megnevezett mezőket (SZA_IG_dátum, Sec_SZA_IG_hely stb.)
     a meglévő heurisztika veszi; itt csak a generikusokat dolgozzuk fel.
  5. Összefésüli az eredményt és elmenti
     src/mapping/V_fuggelek_mapping.json formátumban.

Használat:
    python3 scripts/recognize_v_fuggelek.py [--pdf V_fuggelek.pdf]
...
logger = logging.getLogger("recognize_v_fuggelek")
logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)-7s %(message)s", datefmt="%H:%M:%S")

DEFAULT_PDF = PROJECT_ROOT / "V_fuggelek.pdf"
DEFAULT_OUT = PROJECT_ROOT / "src" / "mapping" / "V_fuggelek_mapping.json"
...
def extract_fields_with_position(pdf_path: Path) -> list[dict]:
    ...
    def walk(f, parent_name: str = ""):
        ...
        try:
            ...
            # Ha van szülő, a fully-qualified név parent.kid.
            ...
            rect = None
            # Widget lookup a lokális (gyerek) névvel is, mert a fitz
            # a kid /T-jét adja vissza field_name-ként.
...
def gather_context_for_fields(doc, fields: list[dict]) -> None:
    ...
    Minden mezőhöz kigyűjti:
      - `nearby`: a mező rect-je körüli 60 pontos sugarú szövegek,
    ...
    for f in fields:
        ...
        if 0 <= page_idx < len(doc):
            page_text = doc[page_idx].get_text("text")[:800]
...
def classify_batch(batch: list[dict], page_no: int, page_text: str) -> dict[str, dict]:
    ...
    system = (
        ...
        "'V. számú függelék - Pénzügyi szolgáltatások közvetítésében való "
        "közreműködésre vonatkozó ügyfél/cég nyilatkozat' PDF AcroForm mezőit "
        "osztályozod. A mezőnevek generikusak (Szövegmező NNN / Jelölőnégyzet NNN), "
        "ezért a `nearby` szöveg és a pozíció alapján döntsd el, mit tartalmaznak.\n\n"
        f"Kanonikus mezők (lehetőségek):\n{CANONICAL_LIST}\n\n"
        "Szabályok:\n"
        "1. CSAK JSON objektumot adj: {\"<pdf_field_name>\": {\"canonical\": \"...|null\", \"type\": \"text|checkbox|date|number\", \"label\": \"rövid magyar címke\"}, ...}\n"
        "2. `canonical` = a legjobban illeszkedő kanonikus mező, vagy `null` ha nem kell adattal kitölteni (pl. dátum/aláírás hely, jogi checkbox).\n"
        "3. Dátum és 'Hely, dátum' mezők → canonical=null, label='Dátum' / 'Hely'.\n"
        "4. Jogi / hozzájáruló jellegű checkbox-ok (pipa dobozok) → canonical=null, type=checkbox, label='Nyilatkozat / hozzájárulás'.\n"
        "5. Ha a mező ügyfél/cég azonosítókra utal (cég neve, cégjegyzékszám, adószám, bankszámlaszám, "
        "kapcsolattartó neve, email, telefon) és a canonical listában nincs megfelelő, akkor canonical=null, de adj részletes label-t.\n"
    )
    ...
    try:
        text = deepseek_chat(
            ...
            system=system, max_tokens=4096, json_mode=True,
        )
        if not text:
            return {}
...
# --- Ismert mezők (a task leírás szerint) ---


KNOWN_NAMED_FIELDS = {
    "SZA_IG_dátum":         ("legal.signature_date", "date", "Igénylő dátum"),
    "SZA_IG_dátum.2":       ("legal.signature_date", "date", "Igénylő dátum (2. példány)"),
    "SZA_IG_hely":          ("legal.signature_place", "text", "Igénylő kelt helye"),
    "SZA_IG_hely.2":        ("legal.signature_place", "text", "Igénylő kelt helye (2. példány)"),
    "Sec_SZA_IG_dátum":     ("legal.signature_date", "date", "Másodlagos aláírás dátum"),
    "Sec_SZA_IG_dátum.4":   ("legal.signature_date", "date", "Másodlagos aláírás dátum (4. példány)"),
    "Sec_SZA_IG_hely":      ("legal.signature_place", "text", "Másodlagos kelt helye"),
    "Sec_SZA_IG_hely.4":    ("legal.signature_place", "text", "Másodlagos kelt helye (4. példány)"),
    "2":                    ("legal.signature_date", "date", "Névtelen dátum mező (2)"),
    "4":                    ("legal.signature_date", "date", "Névtelen dátum mező (4)"),
}


def is_generic(name: str) -> bool:
    return (
        name.startswith("Jelölőnégyzet ") or name.startswith("Szövegmező ")
    )


def main(pdf_path: Path = DEFAULT_PDF, out_path: Path = DEFAULT_OUT) -> None:
    import fitz
    ...
    mapping = MappingConfig(
        bank_name="OTP Bank",
        form_name="V_fuggelek",
        form_type="acroform",
        fields=recognized,
        notes=f"Context-based AI felismerés ({len(recognized)} mező, "
    ...
    print(f"\n=== V függelék mapping kész ===")
    print(f"Összes mező:      {len(recognized)}")
    print(f"Leképezve:        {mapped}")
    print(f"Checkbox (Btn):   {sum(1 for r in recognized if r.field_type == FieldType.CHECKBOX)}")
